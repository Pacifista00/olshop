import { useEffect, useState } from "react";
import api from "../../../services/Api";

import {
  Table,
  TableBody,
  TableCell,
  TableHeader,
  TableRow,
} from "../ui/table";

import Badge from "../ui/badge/Badge";

export default function ProductRedemptionList() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const [currentPage, setCurrentPage] = useState(1);
  const [lastPage, setLastPage] = useState(1);

  const [showDetailModal, setShowDetailModal] = useState(false);
  const [selectedItem, setSelectedItem] = useState(null);

  const [trackingNumber, setTrackingNumber] = useState("");
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    fetchData(currentPage);
  }, [currentPage]);

  const fetchData = async (page = 1) => {
    try {
      setLoading(true);

      const response = await api.get(`/admin/redemptions/product?page=${page}`);
      const res = response.data;

      setData(res.data || []);
      setCurrentPage(res.meta?.current_page || 1);
      setLastPage(res.meta?.last_page || 1);
    } catch (err) {
      console.error(err);
      setError("Gagal memuat data penukaran produk");
    } finally {
      setLoading(false);
    }
  };

  const openDetailModal = (item) => {
    setSelectedItem(item);
    setTrackingNumber(item.tracking_number || "");
    setShowDetailModal(true);
  };

  const closeDetailModal = () => {
    setSelectedItem(null);
    setTrackingNumber("");
    setShowDetailModal(false);
  };

  const handleComplete = async () => {
    if (!trackingNumber) {
      alert("Tracking number wajib diisi");
      return;
    }

    try {
      setSubmitting(true);

      await api.post(`/admin/redemptions/product/${selectedItem.id}/complete`, {
        tracking_number: trackingNumber,
      });

      alert("Berhasil menyimpan resi");

      // 🔥 update state tanpa reload
      setData((prev) =>
        prev.map((item) =>
          item.id === selectedItem.id
            ? {
                ...item,
                tracking_number: trackingNumber,
                status: "completed",
              }
            : item,
        ),
      );

      closeDetailModal();
    } catch (err) {
      console.error(err);
      alert("Gagal menyimpan resi");
    } finally {
      setSubmitting(false);
    }
  };

  const statusColor = (status) => {
    switch (status) {
      case "completed":
        return "success";
      case "processing":
        return "warning";
      case "shipped":
        return "info";
      case "cancelled":
        return "danger";
      default:
        return "secondary";
    }
  };

  if (loading) {
    return (
      <div className="p-5 text-gray-500 text-sm">
        Loading data penukaran produk...
      </div>
    );
  }

  if (error) {
    return <div className="p-5 text-red-500 text-sm">{error}</div>;
  }

  return (
    <>
      {/* ================= TABLE ================= */}
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableCell isHeader>Resi</TableCell>
                <TableCell isHeader>Produk</TableCell>
                <TableCell isHeader>User</TableCell>
                <TableCell isHeader>Penerima</TableCell>
                <TableCell isHeader>Status</TableCell>
                <TableCell isHeader>Tanggal</TableCell>
                <TableCell isHeader>Aksi</TableCell>
              </TableRow>
            </TableHeader>

            <TableBody>
              {data.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={6}
                    className="text-center py-6 text-gray-400"
                  >
                    Tidak ada data
                  </TableCell>
                </TableRow>
              ) : (
                data.map((item) => (
                  <TableRow key={item.id}>
                    <TableCell>
                      <p className="font-medium">{item.tracking_number}</p>
                    </TableCell>
                    <TableCell>
                      <p className="font-medium">{item.product_name}</p>
                    </TableCell>

                    <TableCell>{item.user?.name || "-"}</TableCell>

                    <TableCell>
                      <div>
                        <p>{item.recipient_name}</p>
                        <p className="text-xs text-gray-400">
                          {item.phone || "-"}
                        </p>
                      </div>
                    </TableCell>

                    <TableCell>
                      <Badge size="sm" color={statusColor(item.status)}>
                        {item.status}
                      </Badge>
                    </TableCell>

                    <TableCell>
                      {new Date(item.created_at).toLocaleDateString("id-ID")}
                    </TableCell>

                    <TableCell>
                      <button
                        onClick={() => openDetailModal(item)}
                        className="text-blue-600 hover:underline text-sm"
                      >
                        Detail
                      </button>
                    </TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
          </Table>
        </div>
      </div>

      {/* ================= PAGINATION ================= */}
      <div className="mt-4 flex items-center justify-between px-2">
        <span className="text-sm text-gray-500">
          Halaman {currentPage} dari {lastPage}
        </span>

        <div className="flex gap-2 flex-wrap">
          <button
            disabled={currentPage === 1}
            onClick={() => setCurrentPage((prev) => prev - 1)}
            className="rounded-lg border px-3 py-1 text-sm disabled:opacity-50"
          >
            Sebelumnya
          </button>

          {Array.from({ length: lastPage }, (_, i) => i + 1).map((page) => (
            <button
              key={page}
              onClick={() => setCurrentPage(page)}
              className={`rounded-lg px-3 py-1 text-sm ${
                page === currentPage
                  ? "bg-blue-600 text-white"
                  : "border hover:bg-gray-100"
              }`}
            >
              {page}
            </button>
          ))}

          <button
            disabled={currentPage === lastPage}
            onClick={() => setCurrentPage((prev) => prev + 1)}
            className="rounded-lg border px-3 py-1 text-sm disabled:opacity-50"
          >
            Berikutnya
          </button>
        </div>
      </div>

      {/* ================= MODAL DETAIL ================= */}
      {showDetailModal && selectedItem && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6">
            <h3 className="text-lg font-semibold text-gray-800">
              Detail Penukaran Produk
            </h3>

            <div className="mt-4 space-y-3 text-sm text-gray-700">
              <div>
                <b>Produk:</b> {selectedItem.product_name}
              </div>

              <div>
                <b>User:</b> {selectedItem.user?.name || "-"}
              </div>

              <div>
                <b>Penerima:</b> {selectedItem.recipient_name}
              </div>

              <div>
                <b>No HP:</b> {selectedItem.phone || "-"}
              </div>

              <div>
                <b>Alamat:</b> {selectedItem.address}
              </div>

              <div>
                <b>Resi:</b>
                <input
                  type="text"
                  value={trackingNumber}
                  onChange={(e) => setTrackingNumber(e.target.value)}
                  className="mt-1 w-full rounded-lg border px-3 py-2 text-sm"
                  placeholder="Masukkan nomor resi / booking code"
                />
                {selectedItem.tracking_number && (
                  <p className="text-xs text-green-600 mt-1">
                    Resi sudah tersimpan
                  </p>
                )}
              </div>

              <div>
                <b>Status:</b>{" "}
                <Badge size="sm" color={statusColor(selectedItem.status)}>
                  {selectedItem.status}
                </Badge>
              </div>

              <div>
                <b>Tanggal:</b>{" "}
                {new Date(selectedItem.created_at).toLocaleString("id-ID")}
              </div>
            </div>

            <div className="mt-6 flex justify-between">
              <button
                onClick={handleComplete}
                disabled={submitting}
                className="rounded-lg bg-green-600 px-4 py-2 text-sm text-white disabled:opacity-50"
              >
                {submitting
                  ? "Menyimpan..."
                  : selectedItem.status === "completed"
                    ? "Update Resi"
                    : "Selesaikan"}
              </button>

              <button
                onClick={closeDetailModal}
                className="rounded-lg border px-4 py-2 text-sm"
              >
                Tutup
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
